...
covid_1_scale = covid_1 %>% 
  select(vaccines_NA,vaccines_count_p,tests_p,recovered_p,deaths_p,
...
loading_1 = data.frame(as.matrix(pca_1$loadings)[1:11,])
loading_2 = data.frame(as.matrix(pca_2$loadings)[1:11,])
loading_3 = data.frame(as.matrix(pca_3$loadings)[1:11,])
loading_4 = data.frame(as.matrix(pca_4$loadings)[1:11,])
loading_5 = data.frame(as.matrix(pca_5$loadings)[1:11,])


loading_avg = Reduce(`+`, mget(paste0("loading_", 1:5)))/5

kbl(loading_avg,caption = "Average Principal Components for 5 data sets",digits = 3) %>% 
...
cum.var = rbind(cum.var_1,cum.var_2,cum.var_3,cum.var_4,cum.var_5)

kbl(cum.var,caption = "Cumulative variance for 5 data sets",digits = 3) %>% 
  kable_classic(full_width = F)
```
...
From the output of hierarchical clustering result of different imputation data sets, about 77.7% of the observations have common clusters in all five data sets.

```{r}
hc_result = covid_1 %>% select(id, administrative_area_level_1, date2) %>% 
  mutate(hc_1 = factor(cut_1),
         hc_2 = factor(cut_2),
         hc_3 = factor(cut_3),
         hc_4 = factor(cut_4),
         hc_5 = factor(cut_5))
...
for(i in 1:nrow(hc_result)){
  if(hc_result$hc_1[i] == "2"){
    hc_1_re[i] = 3
  }else if(hc_result$hc_1[i] == "3"){
    hc_1_re[i] = 2
  }else{
    hc_1_re[i] = 1
  }
...
hc_result2 %>% filter(max==5) %>% nrow()/990
...
covid_1_scale_hc = cbind(covid_1_scale,hc_result$hc_1) %>% as.data.frame() %>% 
  mutate(cluster = factor(V12))

covid_2_scale_hc = cbind(covid_2_scale,hc_result$hc_2) %>% as.data.frame() %>% 
  mutate(cluster = factor(V12))

covid_3_scale_hc = cbind(covid_3_scale,hc_result$hc_3) %>% as.data.frame() %>% 
  mutate(cluster = factor(V12))

covid_4_scale_hc = cbind(covid_4_scale,hc_result$hc_4) %>% as.data.frame() %>% 
  mutate(cluster = factor(V12))

covid_5_scale_hc = cbind(covid_5_scale,hc_result$hc_5) %>% as.data.frame() %>% 
  mutate(cluster = factor(V12))

auto1 = autoplot(pca_1, data = covid_1_scale_hc, colour = "cluster", size = 3,
         main = "(A) PCA for covid 1 data set") + 
...
auto2 = autoplot(pca_2, data = covid_2_scale_hc, colour = "cluster",size = 3,
           main = "(B) PCA for covid 2 data set") + 
  scale_color_discrete(name = "Cluster", labels=c("Cluster 1","Cluster 2","Cluster 3")) +
  labs(x = "Comp.1", y = "Comp.2") +
  theme_bw() +
  theme(plot.title = element_text(size = 26, face = "bold"),
...
auto3 = autoplot(pca_3, data = covid_3_scale_hc, colour = "cluster",size = 3,
           main = "(C) PCA for covid 3 data set") + 
  scale_color_discrete(name = "Cluster", labels=c("Cluster 1","Cluster 2","Cluster 3")) +
  labs(x = "Comp.1", y = "Comp.2") +
  theme_bw() +
  theme(plot.title = element_text(size = 26, face = "bold"),
...
auto4 = autoplot(pca_4, data = covid_4_scale_hc, colour = "cluster",size = 3,
           main = "(D) PCA for covid 4 data set") + 
  scale_color_discrete(name = "Cluster", labels=c("Cluster 1","Cluster 2","Cluster 3")) +
  labs(x = "Comp.1", y = "Comp.2") +
  theme_bw() +
  theme(plot.title = element_text(size = 26, face = "bold"),
...
auto5 = autoplot(pca_5, data = covid_5_scale_hc, colour = "cluster",size = 3,
           main = "(E) PCA for covid 5 data set") +
  scale_color_discrete(name = "Cluster", labels=c("Cluster 1","Cluster 2","Cluster 3")) +
  labs(x = "Comp.1", y = "Comp.2") +
  theme_bw() +
  theme(plot.title = element_text(size = 26, face = "bold"),
...
auto1+auto2+auto3+auto4+auto5+plot_layout(ncol = 5)
